...
const fallbackRepoUrl = 'https://github.com/eddielueng/gptimage';
const originalRepoUrl = 'https://github.com/freestylefly/awesome-gpt-image-2';
...
const copy = {
  en: {
    ...
    loginButton: 'Login',
    registerSuccess: 'Registration successful! Please check your email to verify, then log in.',
    ...
    apiKeyHint: 'With your own API key, image generation uses your key directly — no credits needed.',
    apiKeySaved: 'API Key saved.',
    apiKeyClear: 'Clear API Key',
    apiKeyRequired: 'Please fill in your API Key to generate images.',
    apiKeyRequiredAction: 'Get your API Key',
    signOut: 'Sign out',
    account: 'Account',
  ...
  zh: {
    ...
    registerButton: '注册',
    loginButton: '登录',
    registerSuccess: '注册成功！请查收邮箱验证后登录。',
    ...
    orDivider: '或',
    apiKeyLabel: 'API Key',
    apiKeyPlaceholder: '输入你的 OpenAI 兼容 API Key',
    apiKeyHint: '使用自己的 API Key，生图直接调用你的 Key，无需消耗积分。',
    apiKeySaved: 'API Key 已保存。',
    apiKeyClear: '清除 API Key',
    apiKeyRequired: '请先填写你的 API Key 才能生成图片。',
    apiKeyRequiredAction: '获取 API Key',
    signOut: '退出登录',
    account: '账号',
...
function generationErrorMessage(error, language) {
  ...
  if (error === 'GENERATION_FAILED') return t.generationFailed;
  // Show the actual API error message for debugging
  if (error && typeof error === 'string' && error.length > 0) return error;
...
function AuthModal({ open, language, onClose }) {
  ...
  function validateEmail() {
    if (!email.trim()) { setMessage(t.emailRequired); return false; }
    return true;
  }

  function validatePassword() {
    if (!password) { setMessage(t.passwordRequired); return false; }
    if (password.length < 6) { setMessage(t.passwordTooShort); return false; }
    return true;
  }
  ...
  async function handleEmailLogin(e) {
    e.preventDefault();
    setMessage('');
    if (!validateEmail() || !validatePassword()) { setStatus('error'); return; }
  ...
  async function handleEmailRegister(e) {
    ...
    setMessage('');
    if (!validateEmail() || !validatePassword()) { setStatus('error'); return; }
    if (password !== confirmPassword) { setStatus('error'); setMessage(t.passwordMismatch); return; }
    if (!isSupabaseConfigured || !supabase) {
      setStatus('error');
      setMessage(t.authNotConfigured);
      return;
    ...
    if (error) {
      setStatus('error');
      setMessage(error.message || t.registerFailed);
    } else {
      setStatus('success');
    ...
    >
      <section className="authDialog" role="dialog" aria-modal="true" aria-labelledby="auth-title">
        ...
        <div className="authTabs">
          <button className={cx('authTab', tab === 'login' && 'active')} type="button" onClick={() => { setTab('login'); setMessage(''); setStatus('idle'); }}>{t.loginTab}</button>
          <button className={cx('authTab', tab === 'register' && 'active')} type="button" onClick={() => { setTab('register'); setMessage(''); setStatus('idle'); }}>{t.registerTab}</button>
        </div>
        ...
        {tab === 'login' ? (
          <form className="authForm" onSubmit={handleEmailLogin}>
            ...
            <button className="authSubmitButton" type="submit" disabled={status === 'loading'}>
              {status === 'loading' ? <LoaderCircle className="spinIcon" size={16} /> : <LogIn size={16} />}
              {t.loginButton}
            </button>
          </form>
        ) : (
          <form className="authForm" onSubmit={handleEmailRegister}>
            <label>
              <span>{t.emailLabel}</span>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" />
            </label>
...
function AccountPanel({
  ...
  function handleSaveApiKey() {
    ...
    window.setTimeout(() => setApiKeyMessage(''), 2400);
  }
  ...
  // Merge local generation records
  let localRecords = [];
  try {
    localRecords = JSON.parse(localStorage.getItem('generation_records') || '[]');
  } catch { /* ignore */ }
  ...
  const generationTransactions = allTransactions.filter((transaction) => transaction.type === 'generation');
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (profile) {
        onProfileChange({ ...profile, fullName: nextName });
      }
      localStorage.setItem('user_full_name', nextName);
      setStatus('success');
      setMessage(t.profileSaved);
    }
  }
      ...
      <section className="accountDialog" role="dialog" aria-modal="true" aria-labelledby="account-title">
        ...
        <div className="accountGrid">
            ...
            <div className="apiKeySection">
              ...
              <div className="apiKeyActions">
                <button type="button" className="portalButton" onClick={handleSaveApiKey} disabled={apiKeyStatus === 'loading'}>
                  {apiKeyStatus === 'loading' ? <LoaderCircle className="spinIcon" size={14} /> : <Check size={14} />}
                  {t.saveProfile}
                </button>
                {userApiKey ? (
                  <button type="button" className="portalButton" onClick={handleClearApiKey} style={{marginLeft: 8}}>
                    <X size={14} />
                    {t.apiKeyClear}
                  </button>
                ) : null}
              ...
              {apiKeyMessage ? (
                <p className={cx('authMessage', apiKeyStatus === 'success' && 'sent', apiKeyStatus === 'error' && 'error')}>
                  {apiKeyMessage}
                </p>
              ) : null}
              <label className="modelSelectLabel">
                <span>{language === 'zh' ? '生图模型' : 'Image Model'}</span>
                <select
                  value={localStorage.getItem('image_model') || import.meta.env.VITE_IMAGE_MODEL || 'gpt-image-2'}
                  onChange={(e) => {
                    localStorage.setItem('image_model', e.target.value);
                    setApiKeyStatus('success');
                    setApiKeyMessage(language === 'zh' ? '模型已切换' : 'Model switched');
                    window.setTimeout(() => setApiKeyMessage(''), 2000);
                  }}
                >
          ...
          <section className="accountOverview">
            ...
            <a className="portalButton accountBillingButton" href="https://xidaoapi.com" target="_blank" rel="noreferrer">
              <CreditCard size={16} />
              {t.membershipCenter}
            </a>
          </section>
...
function PreviewDialog({
  ...
  const hasUserApiKey = Boolean(localStorage.getItem('user_api_key'));
  const isOutOfCredits = !hasUserApiKey
    && isSignedIn
    && creditBalance <= 0
    && (profile?.isSuperAdmin || Boolean(profile?.freeUsed));
  const generationLocked = isGenerating;
  const quotaText = hasUserApiKey
    ? (language === 'zh' ? '使用你的 API Key 生成' : 'Generate with your API Key')
    : t.apiKeyRequired;

  async function handleGenerate() {
    if (isTemplate || isGenerating) return;
    if (!localStorage.getItem('user_api_key')) {
      setGenerationState({ status: 'error', image: '', message: t.apiKeyRequired });
      return;
    }
    ...
    try {
      ...
      if (userApiKey) {
        // Use user's own API key to call the image API directly
        const baseUrl = 'https://hk.xidaoapi.com';
        const response = await fetch(`${baseUrl}/v1/images/generations`, {
          ...
          body: JSON.stringify({
            model: localStorage.getItem('image_model') || import.meta.env.VITE_IMAGE_MODEL || 'gpt-image-2',
            prompt,
            n: 1,
            size: '1024x1024',
            quality: 'low',
        ...
        if (!response.ok || !payload?.data?.[0]?.b64_json) {
          throw new Error(payload?.error?.message || payload?.message || 'GENERATION_FAILED');
        }
        const image = `data:image/jpeg;base64,${payload.data[0].b64_json}`;
        const savedAt = new Date().toISOString();
        saveGeneratedTest(item.id, { image, prompt, savedAt });
        // Save generation record to localStorage
        try {
          ...
          localStorage.setItem('generation_records', JSON.stringify(records.slice(0, 50)));
        } catch { /* ignore */ }
        setGenerationState({ status: 'success', image, message: '', prompt, savedAt });
      }
    } catch (error) {
      setGenerationState({
        status: 'error',
      ...
      <section className="previewDialog" role="dialog" aria-modal="true" aria-labelledby="preview-title">
        ...
        <div className="previewContent">
          ...
          <div className="previewActions">
            ...
            {!isTemplate ? (
              !hasUserApiKey ? (
                <a className="buyCreditsLink" href="https://xidaoapi.com" target="_blank" rel="noreferrer">
                  <Key size={17} />
                  {t.apiKeyRequiredAction}
          ...
          {!isTemplate ? (
            <div className="generationPanel">
              <div className={cx('generationQuota', !hasUserApiKey && 'used')}>
                {quotaText}
              </div>
              {!hasUserApiKey ? (
                <a className="buyCreditsLink" href="https://xidaoapi.com" target="_blank" rel="noreferrer">
...
function App() {
  ...
  useEffect(() => {
    ...
    fetch('/api/me', {
      ...
      .catch(() => {
        // Backend unavailable (local dev without Vercel) — build a fallback profile
        if (!cancelled && session?.user) {
          const adminEmails = (import.meta.env.VITE_SUPER_ADMIN_EMAILS || '').split(',').map(e => e.trim().toLowerCase()).filter(Boolean);
          ...
          setProfile({
            ...
            freeUsed: false,
            membership: null,
            usage: { totalGenerations: 0, totalGenerationCredits: 0 },
            recentTransactions: []
          });
        } else if (!cancelled) {
          setProfile(null);
  ...
  async function loadFavorites({ silent = true } = {}) {
    ...
    try {
      ...
      if (localFavorites.length > 0) {
        setFavoriteRows(normalizeFavoriteRows(localFavorites));
      }
    } catch { /* ignore */ }
    ...
    try {
      ...
      // Return localStorage favorites as fallback
      try {
        return normalizeFavoriteRows(JSON.parse(localStorage.getItem('user_favorites') || '[]'));
      } catch {
        return [];
      }
    }
  ...
  useEffect(() => {
    ...
    if (!session?.access_token) {
      // Not logged in — load local favorites
      try {
        const localFavorites = JSON.parse(localStorage.getItem('user_favorites') || '[]');
        if (localFavorites.length > 0) {
          setFavoriteRows(normalizeFavoriteRows(localFavorites));
  ...
  async function handleToggleFavorite(caseItem) {
    ...
    try {
      const localFavorites = JSON.parse(localStorage.getItem('user_favorites') || '[]');
      if (isFavorite) {
        const filtered = localFavorites.filter((f) => f.caseId !== caseId);
        localStorage.setItem('user_favorites', JSON.stringify(filtered));
      } else {
        localFavorites.unshift({ caseId, createdAt: new Date().toISOString() });
    ...
    if (session?.access_token) {
      try {
        ...
        } else if (!isFavorite && payload.favorite) {
        ...
        // Backend unavailable but local save already succeeded
      }
    }
  ...
  return (
    <main>
      ...
      <footer className="siteFooter">
        <p>
          Powered by <a href="https://xidaoapi.com" target="_blank" rel="noreferrer">XidaoAPI.com</a>
          {' · '}
          Based on <a href={originalRepoUrl} target="_blank" rel="noreferrer">awesome-gpt-image-2</a> by <a href="https://github.com/freestylefly" target="_blank" rel="noreferrer">freestylefly</a>
        </p>
      </footer>
    </main>
  );
